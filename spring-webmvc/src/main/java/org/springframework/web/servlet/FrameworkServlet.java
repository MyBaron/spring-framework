...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	 ...
	 *
	 * 重写HttpServletBean的initServletBean方法，HttpServletBean的init方法会在最后调用这方法
	 */
	@Override
	...
	/**
	 * 此处的创建WebApplicationContext并不是root，而是子的WebApplicationContext
	 *
	...
	protected WebApplicationContext   initWebApplicationContext() {
		// 从ServletContext中获取root的WebApplicationContext，因为root的WebApplicationContext只会被创建一次，创建完后会存在ServletContext中
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// 处理 PATCH 请求，因为PATCH是新增的方法，父类HttpServlet并没有对这个方法进行处理。
		if (httpMethod == HttpMethod.PATCH || httpMethod == null) {
			processRequest(request, response);
		}
		else {
			//调用父类HTTPServlet处理其他方法的请求
